...
exports.createSmartGatewayPayment = async (req, res) => {
  try {
    ...
    const returnUrl = `https://www.bookmyevent.ae/payment-success/index.html?bookingId=${bookingId}`;
  ...
  try {
    ...
    if (booking) {
      if (status === "CHARGED") {
        ...
        console.log("✅ Booking payment marked as COMPLETED");
      }
    ...
    if (!subscription) {
      console.log("⚠️ No booking or subscription found for order:", order_id);
      return res.sendStatus(200);
    }
