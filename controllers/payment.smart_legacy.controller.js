...
//         return_url: `https://bookmyevent.ae/payment-success/index.html?status=success&bookingId=${bookingId}`,
...
console.log("✅ Juspay SDK initialized");
console.log("Merchant:", config.MERCHANT_ID);

// =======================================================
// CREATE PAYMENT SESSION (BOOKING)
// =======================================================
exports.createSmartGatewayPayment = async (req, res) => {
  try {
    const { bookingId } = req.body;
    ...
    // ---------------------------------------------------
    // 3️⃣ FINAL RETURN URL (🔥 SINGLE SOURCE OF TRUTH)
    // ---------------------------------------------------
    const SUCCESS_RETURN_URL =
      `https://bookmyevent.ae/payment-success/index.html?status=success&bookingId=${bookingId}`;

    console.log("🔁 RETURN URL:", SUCCESS_RETURN_URL);
    ...
    // ---------------------------------------------------
    // 6️⃣ CLEAN SDK PAYLOAD (IMPORTANT)
    // ---------------------------------------------------
    const sdkPayload = JSON.parse(JSON.stringify(session.sdk_payload));
    if (sdkPayload?.payload?.returnUrl) {
      delete sdkPayload.payload.returnUrl;
