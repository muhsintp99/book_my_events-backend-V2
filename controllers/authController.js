      ...
      vendorProfile = await VendorProfile.findOne({ user: user._id }).populate([
        {
          path: "module",
          select: "moduleId title icon categories isActive createdAt updatedAt",
          populate: { path: "categories", select: "title description isActive createdAt updatedAt" },
        },
        { path: "zone", select: "name description coordinates city country isActive" },
      ]);
    }
...
exports.refreshToken = async (req, res) => {
  try {
    ...
    res.json({ message: 'Token refreshed', token: newAccessToken, refreshToken: newRefreshToken });
  } catch (err) {
    console.error("Refresh Token Error:", err);
    res.status(500).json({ message: 'Token refresh failed', error: err.message });
  }
...
exports.sendOtp = async (req, res) => {
  try {
    ...
    try { await sendEmail(email, 'Your OTP Code', otpEmail(otp)); } 
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const resetURL = `${req.protocol}://${req.get('host')}/api/auth/reset-password/${resetToken}`;
    try { await sendEmail(email, 'Password Reset', resetPasswordEmail(resetURL)); }
