...
exports.getLightAndSoundVendors = async (req, res) => {
    try {
        ...
        ================================= */
        const allPackages = await LightAndSound.find({
            secondaryModule: new mongoose.Types.ObjectId(moduleId),
            provider: { $in: vendorIds },
            isActive: true
        })
        ...
        if (zoneMissing.length > 0) {
            ...
            for (const vendor of zoneMissing) {
                try {
                    ...
                    console.error("Final fallback error for L&S vendor:", vendor._id, err.message);
                }
            }
        }

        // ✅ Filter out vendors with NO packages (as requested)
        ...
        console.error("❌ Get Light and Sound Vendors Error:", err);
        res.status(500).json({ success: false, message: err.message });
    }
};
