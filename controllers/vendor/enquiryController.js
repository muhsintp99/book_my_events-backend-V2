...
exports.createEnquiry = async (req, res) => {
  try {
    // 🔥 AUTO-FILL moduleId & moduleModel if missing (for secondary modules)
    if (req.body.vendorId) {
      const User = require("../../models/User");
      const vendor = await User.findById(req.body.vendorId).populate("vendorProfile");
      
      if (vendor?.vendorProfile) {
          if (!req.body.moduleId && vendor.vendorProfile.module) {
            req.body.moduleId = vendor.vendorProfile.module;
          }
          // Always ensure moduleModel is correct based on vendor profile if not explicitly provided
          if (!req.body.moduleModel && vendor.vendorProfile.moduleType) {
            req.body.moduleModel = vendor.vendorProfile.moduleType;
          }
      }
    }
    ...
    const enquiry = await Enquiry.create(req.body);
    const populated = await Enquiry.findById(enquiry._id)
      .populate({ path: "moduleId" }) // Use dynamic refPath
      .populate(VENDOR_POPULATION)
      .populate("userId", "firstName lastName email");
...
exports.getEnquiriesByProvider = async (req, res) => {
  try {
    const enquiries = await Enquiry.find({ vendorId: req.params.providerId })
      .populate("moduleId") // Populates based on refPath
      .populate(VENDOR_POPULATION)
      .populate("userId", "firstName lastName email")
      .sort({ createdAt: -1 });

    // Fetch vendor info for fallback
    const User = require("../../models/User");
    const vendor = await User.findById(req.params.providerId).populate({
        path: "vendorProfile",
        populate: { path: "module" }
    });
    const vendorModule = vendor?.vendorProfile?.module;

    const data = await Promise.all(
      enquiries.map(async (e) => {
        let obj = e.toObject();
        
        // Fallback: If moduleId didn't populate (old data or missing ref), use vendor's primary module
        if (!obj.moduleId && vendorModule) {
            obj.moduleId = {
                _id: vendorModule._id,
                title: vendorModule.title,
                icon: vendorModule.icon
            };
        }

        return {
          ...obj,
          packageDetails: await resolvePackageDetails(obj.moduleId?.title, obj.packageId),
        };
      })
    );
    res.json({ success: true, data });
