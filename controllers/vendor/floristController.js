...
/* =====================================================
   CREATE PACKAGE
===================================================== */
exports.createFloristPackage = async (req, res) => {
    try {
        const {
            secondaryModule,
            module,
        ...
        const packageId = `FLR-${Date.now()}`;

        // Handle thumbnail
        let thumbnail = null;
        if (req.files && req.files.thumbnail && req.files.thumbnail[0]) {
            thumbnail = `/uploads/florist/${req.files.thumbnail[0].filename}`;
        }

        // Handle multiple images
        let images = [];
        if (req.files && req.files.images) {
            images = req.files.images.map(file => `/uploads/florist/${file.filename}`);
        }

        const pkg = await Florist.create({
            packageId,
        ...
        res.status(201).json({
            success: true,
            message: "Florist package created successfully",
            data: populatedPkg,
        });
...
===================================================== */
exports.getAllFloristPackages = async (req, res) => {
    try {
        const {
            keyword,
            moduleId,
        ...
        const packages = await Florist.aggregate(dataPipeline);

        const countPipeline = [...pipeline, { $count: "total" }];
        const countResult = await Florist.aggregate(countPipeline);
...
===================================================== */
exports.getFloristPackageById = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const pkg = await Florist.findById(id)
            .populate({
                path: "provider",
...
===================================================== */
exports.getFloristByVendor = async (req, res) => {
    try {
        ...
        const packages = await Florist.find(query)
            .populate({
                path: "provider",
                select: "firstName lastName email phone profilePhoto",
                populate: {
...
exports.getFloristVendors = async (req, res) => {
    try {
        ...
        ================================= */
        const allPackages = await Florist.find({
            secondaryModule: new mongoose.Types.ObjectId(moduleId),
            provider: { $in: vendorIds },
            isActive: true
        })
        ...
        if (zoneMissing.length > 0) {
            ...
            for (const vendor of zoneMissing) {
                try {
                    ...
                    console.error("Final fallback error for florist vendor:", vendor._id, err.message);
                }
            }
        }
        ...
        console.error("❌ Get Florist Vendors Error:", err);
        res.status(500).json({ success: false, message: err.message });
    }
};
...
exports.updateFloristPackage = async (req, res) => {
    try {
        const pkg = await Florist.findById(req.params.id);
        if (!pkg)
            return res.status(404).json({ success: false, message: "Package not found" });
        ...
        if (req.files && req.files.thumbnail && req.files.thumbnail[0]) {
            ...
            pkg.thumbnail = `/uploads/florist/${req.files.thumbnail[0].filename}`;
        }
        ...
        if (req.files && req.files.images && req.files.images.length > 0) {
            ...
            pkg.images = req.files.images.map(file => `/uploads/florist/${file.filename}`);
        }
...
/* =====================================================
   DELETE PACKAGE
===================================================== */
exports.deleteFloristPackage = async (req, res) => {
    try {
        const pkg = await Florist.findById(req.params.id);
        if (!pkg)
            return res.status(404).json({ success: false, message: "Package not found" });
