...
/* =====================================================
   CREATE PACKAGE
===================================================== */
exports.createEmceePackage = async (req, res) => {
    try {
        const {
            secondaryModule,
            module,
        ...
        const packageId = `EMC-${Date.now()}`;

        const image = req.file
            ? `/Uploads/emcee/${req.file.filename}`
            : null;

        const pkg = await Emcee.create({
            packageId,
        ...
        res.status(201).json({
            success: true,
            message: "Emcee package created successfully",
            data: populatedPkg,
        });
...
===================================================== */
exports.getAllEmceePackages = async (req, res) => {
    try {
        const {
            keyword,
            moduleId,
        ...
        const packages = await Emcee.aggregate(dataPipeline);

        const countPipeline = [...pipeline, { $count: "total" }];
        const countResult = await Emcee.aggregate(countPipeline);
...
===================================================== */
exports.getEmceePackageById = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const pkg = await Emcee.findById(id)
            .populate({
                path: "provider",
...
===================================================== */
exports.getEmceeByVendor = async (req, res) => {
    try {
...
===================================================== */
exports.getEmceeVendors = async (req, res) => {
    try {
        ...
        if (zoneMissing.length > 0) {
            ...
            for (const vendor of zoneMissing) {
                try {
                    ...
                    console.error("Final fallback error for emcee vendor:", vendor._id, err.message);
                }
            }
        }
...
/* =====================================================
   UPDATE PACKAGE
===================================================== */
exports.updateEmceePackage = async (req, res) => {
    try {
        const pkg = await Emcee.findById(req.params.id);
        if (!pkg)
            return res.status(404).json({ success: false, message: "Package not found" });
        ...
        if (req.file) {
            ...
            pkg.image = `/Uploads/emcee/${req.file.filename}`;
        }
...
/* =====================================================
   DELETE PACKAGE
===================================================== */
exports.deleteEmceePackage = async (req, res) => {
    try {
        const pkg = await Emcee.findById(req.params.id);
        if (!pkg)
            return res.status(404).json({ success: false, message: "Package not found" });
